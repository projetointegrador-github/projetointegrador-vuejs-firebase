<template>
    <v-app>
    <v-app-bar app color="black" dark elevate-on-scroll elevation="2" id="header">
        <Tabs />
        <v-img src="../assets/logoLoja.png" max-width="70px" id="logo"></v-img>
        <v-spacer></v-spacer>
        <div class="accountDiv d-flex align-center">
            <v-btn icon :to="{name: 'Perfil'}"><v-icon size="2.1em">mdi-account-circle-outline</v-icon></v-btn>
            <v-btn icon :to="{name: 'Carrinho'}"><v-icon size="2.1em">mdi-cart-variant</v-icon></v-btn>
